import { clients, sendOffer, dataChannel } from "./webrtc"



const sendOfferBtn = <HTMLButtonElement | null> document.querySelector('.send-offer')
const sendMsgBtn = <HTMLButtonElement | null> document.querySelector('.send-msg')
const msgInput = <HTMLTextAreaElement | null> document.querySelector('textarea')
const receivedMsg = <HTMLTextAreaElement | null> document.querySelector('.received-msg')

if(!sendOfferBtn || !sendMsgBtn || !msgInput || !receivedMsg) throw new Error('Not found send-offer button.')
